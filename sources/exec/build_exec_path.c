...
/*   Updated: 2024/06/26 15:30:36 by jean-michel      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int exec_found(const char *dirname, char *exec_searched) {
    ...
    if (!dir) {
        if (errno == 2)
            return (0);
        return (handle_error("opendir", errno), -1);
    }
    entity = readdir(dir);
    if (!entity && errno)
        return (handle_error("readdir", errno), closedir(dir), -1);
    while (entity != NULL)
	{
        if (ft_strcmp(entity->d_name, exec_searched) == 0)
		{
            if (closedir(dir) == -1)
                return (handle_error("closedir", errno), -1);
            return (1);
        }
        entity = readdir(dir);
        if (!entity && errno) {
